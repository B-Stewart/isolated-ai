...
When invoked:
1. Detect the test framework: check `package.json` (jest, vitest, mocha), `pyproject.toml` (pytest), `Cargo.toml` (cargo test), `.csproj` / `.fsproj` (xUnit, NUnit, MSTest — look at `<PackageReference>` entries), etc. Read one existing test file to match conventions (file location, naming, assertion style).
